...
import { TRANSFER_TRANSACTION_MODEL, TransferTransactionView } from '../read-model/schema/transfer-transaction.schema';
@Injectable()
export class TransactionService {
  ...
  async getTransferTransactionsBySender(idSender: string): Promise<TransferTransactionView[]> {
    return this.transferTransactionModel.find({ 'idSender': "" + idSender + "" }).exec();
  }

  async getTransferTransactionsByBeneficiary(idBeneficiary: string): Promise<TransferTransactionView[]> {
    return this.transferTransactionModel.find({ 'idBeneficiary': "" + idBeneficiary + "" }).exec();
  }
}